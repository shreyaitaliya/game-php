    ...
    <main class="d-flex">
        ...
        <div class="main w-full px-12 py-6">
            <div class="games-list">
                <a href="<?php echo $site_url ?>admin/add-page" class="py-2 px-6 bg-blue-400 text-white uppercase text-xs rounded-md">ADD</a>
                    ...
                    <tbody class="py-4">
                        ...
                        <?php while ($row = mysqli_fetch_assoc($run)) { ?>
                            <tr class="bg-[white] dark:bg-zinc-900 px-16 py-4 rounded-lg">
                                ...
                                <td class="text-gray-500 text-xs px-6 py-4"><a href="add-page?action=update&token_id=<?=$row['id']?>"><?=$row['title']?></a></td>
                                <td class="text-right relative px-6">
                                    ...
                                    <div id="dc_<?= $row['id'] ?>" style="z-index: 99;" class="dropdown absolute bg-white text-right right-0 hidden flex-column ">
                                        <a href="functions/functions.php?action=delete&token_id=<?= $row['id'] ?>&content_type=page&url=<?php echo $site_url ?>admin/pages" class="text-xs px-4 py-2 text-red-700">Delete</a>
