                            ...
                            <tr class="bg-[white] dark:bg-zinc-900 px-16 <?php if($row['status'] == 1) { echo "border-2 border-red-800 "; } ?> ">
                                ...
                                <td class="text-right relative px-6">
                                    ...
                                    <div id="dc_<?= $row['id'] ?>" style="z-index: 99;" class="dropdown absolute bg-white text-right right-0 hidden flex-column ">
                                        <a href="functions/functions.php?action=delete&token_id=<?= $row['id'] ?>&content_type=user&url=<?php echo $site_url ?>admin/index" class="text-xs px-4 py-2 text-red-700">Delete</a>
                                        <a href="add-users.php?action=update&token_id=<?= $row['id'] ?>&content_type=game" class="text-xs px-4 py-2">Edit</a>
                                    </div>
                                </td>
                            </tr>
                        <?php } ?>
